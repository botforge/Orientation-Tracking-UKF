...
def process_imu(imu_raw, rpy=False):
    ...
    return np.hstack((accel, gyro)), delta_t

def estimate_rot(data_num=1, plot=False):
    imu_raw, vicon_raw = load_data(data_num)
    plot_vicon(vicon_raw)
    imu_data, delta_t = process_imu(imu_raw, rpy=False)
    data_len = imu_data.shape[0]

    ukf = UKF()
    for i in range(data_len-1):
        W_prime, Y = ukf.process_update(delta_t[i])
        ukf.measurement_update(imu_data[i+1, :], W_prime, Y)
        print("i:", i, ", TRACE COV:", np.trace(ukf.P))
